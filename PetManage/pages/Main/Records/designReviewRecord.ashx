     ...
     private string RecordPatientInformation(HttpContext context)
    {
        ...
        catch (System.Exception Ex)
        {
            ...
            else
            {
                ...
                //}
                string checkcommand = "select Singledose,Totaldose from fieldinfomation where ChildDesign_ID=@chid";
                sqlOperation2.AddParameterWithValue("@chid", context.Request.Form["childdesign" + ggg]);
                MySql.Data.MySqlClient.MySqlDataReader reader2 = sqlOperation2.ExecuteReader(checkcommand);
                int totalnum = 0;
                if (reader2.Read())
                {
                    string single = reader2["Singledose"].ToString();
                    string total = reader2["Totaldose"].ToString();
                    int singledose = 0;
                    int totaldose = 0;
                    if (single != "")
                    {
                        singledose = int.Parse(single);
                    }
                    if (total != "")
                    {
                        totaldose = int.Parse(total);
                    }
                    if (singledose != 0)
                    {
                        totalnum = totaldose / singledose;
                    }
                }
                reader2.Close();

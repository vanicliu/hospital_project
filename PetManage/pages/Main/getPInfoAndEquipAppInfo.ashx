...
public class getPInfoAndEquipAppInfo : IHttpHandler {
    ...
    private string getpaeinfo(HttpContext context)
    {
        ...
        string designcommand = "select childdesign.ID as chid,DesignName,childdesign.Splitway_ID as splitway,childdesign.Totalnumber as total,childdesign.fillNumber as fillnum,childdesign.state as childstate,Treatmentdescribe,childdesign.Treatment_ID as treatid from treatment,childdesign where childdesign.Treatment_ID=treatment.ID and treatment.Patient_ID=@pid and treatment.Progress not LIKE '%14%' and treatment.Progress like '%12%' and childdesign.state=3  and treatment.ID in (" + str + ")";
        sqlOperation.AddParameterWithValue("@pid", patientid);
        ...
        while (reader.Read())
        {
            ...
            info.Append("{\"chid\":\"" + reader["chid"].ToString() + "\",\"DesignName\":\"" + reader["DesignName"].ToString() + "\",\"isfirst\":\"" + isfirst + "\",\"Totalnumber\":\"" + (int.Parse(reader["total"].ToString()) + int.Parse(reader["fillnum"].ToString())) + "\",\"childstate\":\"" + reader["childstate"].ToString() + "\",\"Treatmentdescribe\":\"" + reader["Treatmentdescribe"].ToString() + "\",\"treatid\":\"" + reader["treatid"].ToString() + "\"");

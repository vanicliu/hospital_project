<%@ WebHandler Language="C#" Class="rankTP" %>
...
public class rankTP : IHttpHandler {
  private string rank;
    ...
    public DataTable GetTP() {
        ...
        connSql.Open();
        string getCommand = "select company,totalprofit from operatingconditions where year='2016'";
        ...
        TA.Columns.Add("totalprofit", typeof(double));
        sda.Fill(TA);
        connSql.Close();
        Object max = TA.Compute("Max(totalprofit)",null);
        double maxD = (double)max;
        Object  min = TA.Compute("Min(totalprofit)", null);
        double minD = (double)min;
        for (int i = 0; i < TA.Rows.Count; i++)
        {
            double totalassets = (double)TA.Rows[i]["totalprofit"];
            double new_ta = (totalassets - minD) / (maxD - minD);
            double new_taD = Math.Round(new_ta, 4);
            TA.Rows[i]["totalprofit"] = new_taD*100;
        }
        DataView dv = TA.DefaultView;
        dv.Sort = "totalprofit desc";
        dv.ToTable();
        return TA;
    }

...
function InitAll() {
    ...
    document.getElementById("select2").addEventListener("change", getsector, false);//自查询
    document.getElementById("clear").addEventListener("click", cle, false);
    $.ajax({
        url: "sectorMange.aspx/AjaxMethod",//发送到本页面后台AjaxMethod方法
        type: "POST",
        dataType: "json",
...
function getyear() {
    ...
    $("#select3").empty();
    $.ajax({
        url: "sectorMange.aspx/AjaxMethod2",//发送到本页面后台AjaxMethod方法
        type: "POST",
        dataType: "json",
        async: true,//async翻译为异步的，false表示同步，会等待执行完成，true为异步
        contentType: "application/json; charset=utf-8",//不可少
...
function getsector() {
    ...
    $.ajax({
        url: "sectorMange.aspx/AjaxMethod5",//发送到本页面后台AjaxMethod方法
        type: "POST",
        dataType: "json",
        async: true,//async翻译为异步的，false表示同步，会等待执行完成，true为异步
        contentType: "application/json; charset=utf-8",//不可少
...
function sel() {
    ...
    $.ajax({
        url: "sectorMange.aspx/AjaxMethod1",//发送到本页面后台AjaxMethod方法
        type: "POST",
        dataType: "json",
        async: true,//async翻译为异步的，false表示同步，会等待执行完成，true为异步
        contentType: "application/json; charset=utf-8",//不可少
        data: "{company:'" + company + "',year:'" + year + "',sector:'" + sector + "'}",
        success: function (data) {
            var jsonData = JSON.parse(data.d); //jsonData是该下路下的所有区间（json格式）
            if (jsonData.olist[0] != "error") {
                var i = 0;
...
function upd() {
    ...
    if (a.replace(/(^\s*)|(\s*$)/g, "") == "" || b.replace(/(^\s*)|(\s*$)/g, "") == "" || c.replace(/(^\s*)|(\s*$)/g, "") == "") {
        alert("企业名称、年份与酒类不可为空");
        return;
    }
    $.ajax({
        url: "sectorMange.aspx/AjaxMethod3",//发送到本页面后台AjaxMethod方法
        type: "POST",
        dataType: "json",
        async: true,//async翻译为异步的，false表示同步，会等待执行完成，true为异步
        contentType: "application/json; charset=utf-8",//不可少
        data: JSON.stringify({ olist: listnow, yeardom: listdom[0], sectordom: listdom[1], companydom: listdom[2] }),
        success: function (data) {
            if (data.d == "1") {
                alert("数据更新成功！");
                location.reload();
...
function cle() {
    ...
    $.ajax({
        url: "sectorMange.aspx/AjaxMethod4",//发送到本页面后台AjaxMethod方法
        type: "POST",
        dataType: "json",
        async: true,//async翻译为异步的，false表示同步，会等待执行完成，true为异步
        contentType: "application/json; charset=utf-8",//不可少

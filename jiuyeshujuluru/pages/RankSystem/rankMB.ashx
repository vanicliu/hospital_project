<%@ WebHandler Language="C#" Class="rankMB" %>
...
public class rankMB : IHttpHandler {
    private string rank;
    ...
    public DataTable GetMB() {
        ...
        connSql.Open();
        string getCommand = "select company,moneyfunds from operatingconditions where year='2016'";
        ...
        TA.Columns.Add("moneyfunds", typeof(double));
        sda.Fill(TA);
        connSql.Close();
        Object max = TA.Compute("Max(moneyfunds)",null);
        double maxD = (double)max;
        Object  min = TA.Compute("Min(moneyfunds)", null);
        double minD = (double)min;
        for (int i = 0; i < TA.Rows.Count; i++)
        {
            double totalassets = (double)TA.Rows[i]["moneyfunds"];
            double new_ta = (totalassets - minD) / (maxD - minD);
            double new_taD = Math.Round(new_ta, 4);
            TA.Rows[i]["moneyfunds"] = new_taD*100;
        }
        DataView dv = TA.DefaultView;
        dv.Sort = "moneyfunds desc";
        dv.ToTable();
        return TA;
    }
